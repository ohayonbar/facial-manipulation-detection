...
def compute_gradient_saliency_maps(samples: torch.tensor, true_labels: torch.tensor, model: nn.Module) -> torch.tensor:
    """Compute vanilla gradient saliency maps for the samples.
    
    Recipe:
        (1) Set requires_grad_ for the samples
        (2) Compute forward pass for samples in model 
        (3) Gather scores corresponding to true labels of samples
        (4) Compute backward pass on these scores
        (5) Collect gradients from samples object
        (6) Compute absolute value (L1) of these values
        (7) Pick maximum value from channels on each pixel
    """
    # 1. Set requires_grad for samples
    samples.requires_grad_()
    
    # 2. Forward pass
    scores = model(samples)
    
    # 3. Gather scores for true labels 
    scores = scores.gather(1, true_labels.view(-1, 1)).squeeze()
    ...
    # 7. Max over channels
    saliency, _ = gradients.max(dim=1)
